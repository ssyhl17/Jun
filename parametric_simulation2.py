...
def run_two_simulation_helper(eplus_run_path, idf_path, output_dir,
                              parameter_one_key, parameter_one_val,
                              parameter_two_key, parameter_two_val):
    """
    ...
    #			 				'solar_heat_gain_coefficient']
    inner_dict = epjson_dict
    for i in range(len(parameter_one_key)):
        if i < len(parameter_one_key) - 1:
            inner_dict = inner_dict[parameter_one_key[i]]

    indexone = parameter_one_key[-1]
    indextwo = parameter_two_key[-1]
    inner_dict[indexone] = parameter_one_val
    inner_dict[indextwo] = parameter_two_val
...
def run_two_parameter_parametric(eplus_run_path, idf_path, output_dir,
    ...
    if not os.path.isdir(output_dir):
        os.mkdir(output_dir)
    result = []
    for para1_value in parameter_one_val:
        for para2_value in parameter_two_val:
            temppath = str(para1_value) + '-' + str(para2_value)
            this_output_dir = output_dir + f'/{temppath}'
            res_dict[temppath] = run_two_simulation_helper(eplus_run_path,
                                                           idf_path,
                                                           this_output_dir,
                                                           parameter_one_key,
                                                           para1_value,
